...
def test_train():
    ...
    rc.create_dataloader(config['files'].getint('num workers', 8))
    ...
    for epoch in range(rc.start_epoch, num_epochs):
        ...
        print('Training accuracy: {:.1f}%/nTraining loss: {}'.format(accuracy.avg, losses.avg), flush=True)
       
        # Validation
        losses, accuracy, confusion = rc.validate()
        print('Validation accuracy: {:.1f}%/nValidation loss: {}'.format(accuracy.avg, losses.avg), flush=True)
        print('Confusion matrix:/n', (confusion), flush=True)
